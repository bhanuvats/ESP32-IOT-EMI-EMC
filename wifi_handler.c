...
bool ip_attach = false;
bool scan_done = false;
char all_scan_results[512];
...
void wifi_cmd_default_handler_sta_connected(void *esp_netif, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ESP_LOGI(APP_TAG, "Wifi connected event");
...
static uint32_t wifi_cmd_get_ap_phy_bitmap(wifi_ap_record_t *ap_record)
{
    ...
    struct {
        ...
        {ap_record->phy_11b, WIFI_PROTOCOL_11B},
        {ap_record->phy_11g, WIFI_PROTOCOL_11G},
        {ap_record->phy_11n, WIFI_PROTOCOL_11N},
#if WIFI_CMD_5G_SUPPORTED
        {ap_record->phy_11a, WIFI_PROTOCOL_11A},
        {ap_record->phy_11ac, WIFI_PROTOCOL_11AC},
#endif
#if WIFI_CMD_HE_SUPPORTED
        {ap_record->phy_11ax, WIFI_PROTOCOL_11AX},
#endif
    };
                                        ...
                                        int32_t event_id, void *event_data)
{
    ...
    if (esp_wifi_scan_get_ap_records(&sta_number, (wifi_ap_record_t *)ap_list_buffer) == ESP_OK) {
        ...
        for (i = 0; i < sta_number; i++) {
            ...
            uint32_t phy_bitmap = wifi_cmd_get_ap_phy_bitmap(&ap_list_buffer[i]);
...
#endif
            size_t used = strlen(all_scan_results);
            size_t remain = 512 - used - 1;
            if (remain > 80) {
                strncat(all_scan_results, scan_result, remain);
            }
